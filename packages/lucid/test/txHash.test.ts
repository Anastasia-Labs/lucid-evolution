import { assert, expect, test } from "vitest";
import {
  Blockfrost,
  Emulator,
  Lucid,
  TxSignBuilder,
  TxSigned,
} from "../src/index.js";
import { NETWORK } from "./specs/services.js";

test("test txHash with defined provider", async () => {
  const projectId = process.env.VITE_BLOCKFROST_KEY_PREPROD;
  const lucid = await Lucid(
    new Blockfrost(process.env.VITE_BLOCKFROST_API_URL_PREPROD!, projectId),
    NETWORK,
  );
  const seedPhrase = process.env.VITE_WALLET_SEED!;
  lucid.selectWallet.fromSeed(seedPhrase);
  const signedTransaction: TxSigned = await lucid
    .fromTx(
      "",
    )
    .sign.withWallet()
    .complete();

  assert(
    signedTransaction.toHash() ===
      "ca41dbd2f3d485def8fe92454cc98d0163aa21abfc7287fc9766d4ce74a2ee04",
  );
});

test.only("test txHash with undefined provider", async () => {
  const lucid = await Lucid();
  const transactionBuilder: TxSignBuilder = lucid.fromTx(
    "",
  );
  assert(
    transactionBuilder.toHash() ===
      "ca41dbd2f3d485def8fe92454cc98d0163aa21abfc7287fc9766d4ce74a2ee04",
  );
});
