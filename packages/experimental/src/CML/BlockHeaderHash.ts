...
) => Effect.Effect<void, BlockHeaderHashError> = Effect.fn(
  (instance: CML.BlockHeaderHash) =>
    Effect.try({
      try: () => instance.free(),
      catch: () =>
...
export const toBech32: (
  instance: CML.BlockHeaderHash,
  prefix: string,
) => Effect.Effect<string, BlockHeaderHashError> = Effect.fn(
...
export const fromBech32: (
  bech32Str: string,
) => Effect.Effect<CML.BlockHeaderHash, BlockHeaderHashError> = Effect.fn(
...
export const toRawBytes: (
  instance: CML.BlockHeaderHash,
) => Effect.Effect<Uint8Array, BlockHeaderHashError> = Effect.fn(
  (instance: CML.BlockHeaderHash) =>
    Effect.try({
