...
) => Effect.Effect<void, ExUnitsError> = Effect.fn((instance: CML.ExUnits) =>
  Effect.try({
    try: () => instance.free(),
    catch: () =>
      new ExUnitsError({
...
export const checkedAdd: (
  instance: CML.ExUnits,
  other: CML.ExUnits,
) => Effect.Effect<CML.ExUnits, ExUnitsError> = Effect.fn(
...
export const toCborBytes: (
  instance: CML.ExUnits,
) => Effect.Effect<Uint8Array, ExUnitsError> = Effect.fn(
  (instance: CML.ExUnits) =>
    Effect.try({
...
) => Effect.Effect<CML.ExUnits, ExUnitsError> = Effect.fn(function* (
  cborBytes: Uint8Array,
) {
...
export const toCborHex: (
  instance: CML.ExUnits,
) => Effect.Effect<string, ExUnitsError> = Effect.fn((instance: CML.ExUnits) =>
  Effect.try({
...
export const toJsValue: (
  instance: CML.ExUnits,
) => Effect.Effect<any, ExUnitsError> = Effect.fn((instance: CML.ExUnits) =>
  Effect.try({
...
export const mem: (
  instance: CML.ExUnits,
) => Effect.Effect<bigint, ExUnitsError> = Effect.fn((instance: CML.ExUnits) =>
  Effect.try({
    try: () => instance.mem(),
    catch: () =>
      new ExUnitsError({
