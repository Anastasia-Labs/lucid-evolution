...
) => Effect.Effect<void, MetadataError> = Effect.fn((instance: CML.Metadata) =>
  Effect.try({
    try: () => instance.free(),
    catch: () =>
      new MetadataError({
 ...
 */
export const _new: () => Effect.Effect<CML.Metadata, MetadataError> = Effect.fn(
  function* () {
 ...
 */
export const _newUnsafe = (): CML.Metadata => Effect.runSync(_new());
...
export const len: (
  instance: CML.Metadata,
) => Effect.Effect<number, MetadataError> = Effect.fn(
  (instance: CML.Metadata) =>
    Effect.try({
      try: () => instance.len(),
      catch: () =>
...
) => Effect.Effect<void, MetadataError> = Effect.fn(
...
export const get: (
  ...
  Effect.fn((instance: CML.Metadata, label: bigint) =>
    Effect.try({
...
export const labels: (
  instance: CML.Metadata,
) => Effect.Effect<CML.TransactionMetadatumLabels, MetadataError> = Effect.fn(
  (instance: CML.Metadata) =>
    Effect.try({
      try: () => instance.labels(),
      catch: () =>
