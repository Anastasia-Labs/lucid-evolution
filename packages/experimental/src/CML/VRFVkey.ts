...
) => Effect.Effect<void, VRFVkeyError> = Effect.fn((instance: CML.VRFVkey) =>
  Effect.try({
    try: () => instance.free(),
    catch: () =>
      new VRFVkeyError({
...
export const toBech32: (
  instance: CML.VRFVkey,
  prefix: string,
) => Effect.Effect<string, VRFVkeyError> = Effect.fn(
...
export const fromBech32: (
  bech32Str: string,
) => Effect.Effect<CML.VRFVkey, VRFVkeyError> = Effect.fn(function* (
  bech32Str: string,
) {
...
export const toRawBytes: (
  instance: CML.VRFVkey,
) => Effect.Effect<Uint8Array, VRFVkeyError> = Effect.fn(
  (instance: CML.VRFVkey) =>
    Effect.try({
...
export const toHex: (
  instance: CML.VRFVkey,
) => Effect.Effect<string, VRFVkeyError> = Effect.fn((instance: CML.VRFVkey) =>
  Effect.try({
    try: () => instance.to_hex(),
    catch: () =>
      new VRFVkeyError({
