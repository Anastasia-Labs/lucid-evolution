...
) => Effect.Effect<void, SingleMintBuilderError> = Effect.fn(
  (instance: CML.SingleMintBuilder) =>
    Effect.try({
      try: () => instance.free(),
      catch: () =>
...
) => Effect.Effect<CML.SingleMintBuilder, SingleMintBuilderError> = Effect.fn(
  function* (assets: CML.MapAssetNameToNonZeroInt64) {
...
export const _newUnsafe = (
  assets: CML.MapAssetNameToNonZeroInt64,
): CML.SingleMintBuilder => Effect.runSync(_new(assets));
 ...
 * @category Constructors
 */
export const newSingleAsset: (
  asset: CML.AssetName,
  amount: bigint,
) => Effect.Effect<CML.SingleMintBuilder, SingleMintBuilderError> = Effect.fn(
  function* (asset: CML.AssetName, amount: bigint) {
    return yield* Effect.try({
 ...
 * @category ConstructorsUnsafe
 */
export const newSingleAssetUnsafe = (
  asset: CML.AssetName,
  amount: bigint,
): CML.SingleMintBuilder => Effect.runSync(newSingleAsset(asset, amount));
...
) => Effect.Effect<CML.MintBuilderResult, SingleMintBuilderError> = Effect.fn(
  (
    instance: CML.SingleMintBuilder,
    _nativeScript: CML.NativeScript,
    witnessInfo: CML.NativeScriptWitnessInfo,
