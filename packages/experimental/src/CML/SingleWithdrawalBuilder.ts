...
) => Effect.Effect<void, SingleWithdrawalBuilderError> = Effect.fn(
  (instance: CML.SingleWithdrawalBuilder) =>
    Effect.try({
      try: () => instance.free(),
      catch: () =>
...
export const _new: (
  ...
  Effect.fn(function* (address: CML.RewardAddress, amount: bigint) {
...
export const _newUnsafe = (
  address: CML.RewardAddress,
  amount: bigint,
): CML.SingleWithdrawalBuilder => Effect.runSync(_new(address, amount));
...
export const paymentKey: (
  ...
  Effect.fn((instance: CML.SingleWithdrawalBuilder) =>
    Effect.try({
