 ...
 * @category Errors
 */
export class WithdrawalBuilderResultError extends Data.TaggedError(
  "WithdrawalBuilderResultError",
)<{
  message?: string;
}> {}
...
) => Effect.Effect<void, WithdrawalBuilderResultError> = Effect.fn(
  (instance: CML.WithdrawalBuilderResult) =>
    Effect.try({
      try: () => instance.free(),
      catch: () =>
