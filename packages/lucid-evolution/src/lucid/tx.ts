...
export class Tx {
  ...
  collectFrom(utxos: UTxO[], redeemer?: Redeemer): Tx {
    this.tasks.push(async (that) => {
      for (const utxo of utxos) {
        ...
        //     ),
        // );
        // const inpt = CML.SingleInputBuilder.from_transaction_unspent_output(coreUtxo)
        // const wit = CML.PlutusScriptWitness.new_script()
        // const pls = CML.PartialPlutusWitness.new()
        // const r = CML.RedeemerSetBuilder.new().add_spend()
        // inpt.plutus_script(CML.PartialPlutusWitness.new(CM))
        // that.txBuilder.add_input()
      }
    });
    return this;
  ...
  //     );
  //     that.txBuilder.add_output(CML.SingleOutputBuilderResult.new(output));
  //   });
  //   return this;
  // }
  ...
  //     const datumOption = (() => {
  //       if (outputData.hash) {
  //       return CML.DatumOption.new_hash(CML.DatumHash.from_hex(outputData.hash))
  //       }
  //       else if (outputData.asHash) {
  //         const plutusData = CML.PlutusData.from_cbor_hex(outputData.asHash)
  ...
  //     // NOTE: this a conditional output class, TransactioOutput or TransactionOutputBuilder
  //     if (outputData.hash) {
  //     const output = CML.TransactionOutput.new(addressFromWithNetworkCheck(address,that.lucid), assetsToValue(assets))
  //       output.set_datum(
  //         C.Datum.new_data_hash(C.DataHash.from_hex(outputData.hash)),
  //       );
  //     } else if (outputData.asHash) {
  //     // const output = CML.TransactionOutputBuilder.new().with_address(addressFromWithNetworkCheck(address, that.lucid))
  ...
  //     }
  //     // that.txBuilder.add_output(output.next().with_value(assetsToValue(assets)));
  //   });
  //   return this;
  // }
