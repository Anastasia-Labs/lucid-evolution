...
const tx = await lucid.newTx().registerPool(poolParams).complete();
...
<Callout type="info" emoji="ℹ️">
  - We're now importing from `@lucid-evolution/lucid` and
  `@lucid-evolution/core-utils` - `lucid.wallet.rewardAddress()` has become
  `lucid.wallet().rewardAddress()` - `tx.sign()` has become
  `tx.sign.withWallet()` - `signWithPrivateKey()` has become
  `sign.withPrivateKey()` - The registerPool function requires a pool deposit.
  It internally creates a pool registration certificate and adds it to the
  transaction.
</Callout>
