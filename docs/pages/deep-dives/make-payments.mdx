...
Lucid Evolution provides several methods for making payments, each with specific use cases. Here's an overview of the available payment methods:
...
Note the change from `payToAddress` to `pay.ToAddress`, and `sign()` to `sign.withWallet()`
...
```typescript
import { fromText } from "@lucid-evolution/utils";
...
Use `pay.ToAddressWithData` to make a payment that includes a datum or script reference.
...
Transition note: `payToAddressWithData`-> `pay.ToAddressWithData`
