...
Our strategy involves a meticulous upgrade process, aligning the library with the latest [Cardano Multiplatform Library (CML)](https://github.com/dcSpark/cardano-multiplatform-lib) versions to address potential compatibility issues and disruptions. 

We propose a holistic approach encompassing maintenance, hard fork compatibility, GitHub issue resolution, pull request management, and community development. Our goal is to create a Cardano ecosystem that is robust, reliable, and adaptable to the evolving needs of its developer base. While the Evolution library shares the Lucid name, it employs a different approach. We've strived to ensure a smooth transition, maintaining similar endpoints and core concepts with only minor API changes.
